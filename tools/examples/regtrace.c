...
const char *regsim8lname = "GPRTracePrint8l";
const char *regsim8hname = "GPRTracePrint8h";
const char *regsim16name = "GPRTracePrint16";
const char *regsim32name = "GPRTracePrint32";
const char *regsim64name = "GPRTracePrint64";

#define QTRACE_REGTRACE_PREINST_GPR 1       /* instrument preinst gpr */
#define QTRACE_REGTRACE_PSTINST_GPR 1       /* instrument pstinst gpr */

#define GPRTracePrint(size, type, name)                           \
static void GPRTracePrint##size(const char* name,                 \
                              type rax,                           \
                              type rcx,                           \
                              type rdx,                           \
                              type rbx,                           \
...
GPRTracePrint(16, uint16_t, regsim16name);
GPRTracePrint(32, uint32_t, regsim32name);
GPRTracePrint(64, uint64_t, regsim64name);
#undef GPRTracePrint

#define GPRTraceInst(position, misc)                                \
static void RegTrace##position##Inst(uint64_t rax,                  \
                                     uint64_t rcx,                  \
                                     uint64_t rdx,                  \
                                     uint64_t rbx,                  \
                                     uint64_t rsp,                  \
...
{                                                                   \
   ...
   GPRTracePrint32(regsim32name, eax, ecx, edx, ebx, esp, ebp, esi,     \
                 ...
                 r15d, eip);                                            \
   GPRTracePrint16(regsim16name, ax, cx, dx, bx, sp, bp, si, di,        \
                 r8w, r9w, r10w, r11w, r12w, r13w, r14w, r15w,          \
                 ip);                                                   \
   GPRTracePrint8(regsim8lname, al, cl, dl, bl, spl, bpl, sil, dil,     \
...
GPRTraceInst(Pst, "pstinst")
#undef GPRTraceInst

void InstructionCallBack(unsigned type)
{
    /// ----------------------------------------------- ///
    /// instrumenting preinst general purpose registers ///
    /// ----------------------------------------------- ///
#if QTRACE_REGTRACE_PREINST_GPR
    QTRACE_INSTRUCTION_INSTRUMENT(QTRACE_BEGIN_ARG,
                                  QTRACE_IPOINT_BEFORE, 
                                  QTRACE_IFUN, RegTracePreInst, 
    ...
    /// ----------------------------------------------- ///
    /// instrumenting pstinst general purpose registers ///
    /// ----------------------------------------------- ///
#if QTRACE_REGTRACE_PSTINST_GPR
    QTRACE_INSTRUCTION_INSTRUMENT(QTRACE_BEGIN_ARG,
                                  QTRACE_IPOINT_AFTER, 
                                  QTRACE_IFUN, RegTracePstInst, 
                                  QTRACE_REGTRACE_VALUE,
                                  QTRACE_X86_RAX,
                                  QTRACE_REGTRACE_VALUE,
                                  QTRACE_X86_RCX,
