...
void cpu_x86_xlate_by_walking_pt(CPUX86State* env, target_ulong addr, 
                                 int is_write1, PageWalkContext *ret)
{
    /* what address to use for xlation */
    target_ulong addr_touse;
    unsigned page_offset= addr & ((1<<TARGET_PAGE_BITS)-1);
    unsigned page_index = (addr >> TARGET_PAGE_BITS) & (CPU_TLB_SIZE - 1);
    unsigned mmu_idx = cpu_mmu_index(env);
    switch (is_write1)
    {
    case QTRACE_PAGE_FETCH:
         addr_touse = env->tlb_table[mmu_idx][page_index].addr_read;
         break;
    case QTRACE_PAGE_WRITE:
         addr_touse = env->tlb_table[mmu_idx][page_index].addr_write;
         break;
    case QTRACE_PAGE_EXEC:
         addr_touse = env->tlb_table[mmu_idx][page_index].addr_code;
         break;
    default:
         break;
    }
    ...
    /* not in tlb. need to walk the page table */
    uint64_t ptep, pte;
    target_ulong pde_addr, pte_addr;
    int error_code, is_dirty, prot, page_size, is_user;
    hwaddr paddr;
...
#endif
    int is_write = (is_write1 == QTRACE_PAGE_WRITE);
    ...
    if (env->cr[4] & CR4_PAE_MASK) {
        ...
        if (env->hflags & HF_LMA_MASK) {
            ...
            if (!(pml4e & PG_PRESENT_MASK)) {
                error_code = 0;
                ret->xlation_fail |= QTRACE_XLATE_NOPRESENT_FAULT;
                goto do_fault;
            }
 ...
 do_mapping:
    ...
    page_offset = addr & ((1<<TARGET_PAGE_BITS)-1);
    paddr = (pte & TARGET_PAGE_MASK) + page_offset;

    ret->paddr = paddr | page_offset;

    /* do not modify the tlb */
    /* tlb_set_page(env, vaddr, paddr, prot, mmu_idx, page_size); */
    return;

    /* we have a fault. we do not handle the fault. */
 do_fault_protect:
    /* we have a permission violation */
    ret->xlation_fail |= QTRACE_XLATE_PERM_FAULT;
 do_fault:
    /* we have a fault */
    return;
}
