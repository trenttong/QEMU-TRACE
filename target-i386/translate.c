...
/* global/shadow register indexes */
static TCGv_ptr cpu_env;
static TCGv_ptr cpu_senv;
static TCGv cpu_A0;
...
/// ------------------------------------------------------------- ///
#define QTRACE_INSTRUMENT_VERIFIED(s)     { s->qtrace_verified = true; }
#define QTRACE_INSTRUMENT_UNVERIFIED(s)   { s->qtrace_verified = false;}

#define QTRACE_INSTRUMENT_CHECK_VERIFIED(s)                             \
do                                                                      \
{                                                                       \
  if (s->module_call_count > 1)    QTRACE_ERROR("CLIENT_MODULE\n");     \
} while(0);
...
{                                                                       \
   ...
   s->module_call_count = 0;                                            \
   QTRACE_RESET_INST_TYPE_FLAG(dc);                                     \
   QTRACE_RESET_INST_RR_FLAG(dc);                                       \
   QTRACE_RESET_INST_WR_FLAG(dc);                                       \
}                                                                       \
while(0);
...
/// ------------------------------------------------ ///
/// QTRACE - X86 instruction TEST definitions.
/// ------------------------------------------------ ///
#define QTRACE_CONSTRUCT_INST_FLAG(F, y, z)  bool TEST_##F(uint64_t flag) { return F & flag; }  
#include "qtrace-api-internal.h"
#undef QTRACE_CONSTRUCT_INST_FLAG

/// ------------------------------------------------ ///
/// QTRACE - X86 instruction register lookup table. 
/// ------------------------------------------------ ///
const QTRACELogItem qtrace_x86_reg_items[] = {
    { 1<<R_EAX     , "RAX" },
    { 1<<R_EBX     , "RBX" },
    { 1<<R_ECX     , "RCX" },
    { 1<<R_EDX     , "RDX" },
...
const QTRACELogItem qtrace_x86_inst_items[] = {
#define QTRACE_CONSTRUCT_INST_FLAG(x, y, z) {x, z},
#include "qtrace-api-internal.h"
#undef QTRACE_CONSTRUCT_INST_FLAG
};
...
static const char* lookup_x86_text(DisasContext *dc, char *disasm, unsigned flags, 
                                   QTRACELogItem *logitems, unsigned logsize)
{
    int i;
    for(i=0;i<=logsize;++i) {
        if (flags & logitems[i].index) {
            strcat(disasm,logitems[i].name);  strcat(disasm,","); 
        }
    }
    strcat(disasm,"done"); 
    return disasm;
...
static const char *lookup_x86_insn_text(DisasContext *dc, char *disasm, unsigned flags)
{
    return lookup_x86_text(dc, disasm, flags, qtrace_x86_inst_items, 54);
}

static const char* lookup_x86_gpr_text(DisasContext *dc, char *disasm, unsigned flags)
{
    return lookup_x86_text(dc, disasm, flags, qtrace_x86_reg_items, R_R15);
}
...
{
    tcg_gen_movi_tl(cpu_tmp0, val);
    tcg_gen_st_tl(cpu_tmp0, cpu_env, offsetof(CPUX86State, physical_eip));
}
...
static unsigned qtrace_get_reg_offset_and_size(DisasContext *s, 
                                               unsigned  regnum, 
                                               uint64_t *offset,
                                               uint64_t *rmsize) 
{
...
pceip_offset:
    ...
    case QTRACE_X86_CS:
         ...
         *offset = offsetof(CPUX86State, segs[R_CS].base);
         break;
    ...
    case R_PRIP :
         *rmsize = sizeof(target_ulong)*8;
         *offset = offsetof(CPUX86State, physical_eip);
         break;
    /* control register */
    case R_CR0 :
    case R_CR1 :
...
static void qtrace_create_preinst_reg_shadow(InstrumentContext *ictx,  
                                             uint64_t offset,
                                             uint64_t rmsize)
{
...
ppctrace_done:
            ...
            case QTRACE_REGTRACE_VALUE_STORE:
                 /* is this a fetch or store operation */
                 icontext->iargs[idx] = QTRACE_REGTRACE_REGFETCH(icontext->iargs[idx]) ?
                                        QTRACE_ICONTEXT_OPERATOR_UNARYFETCH : 
                                        QTRACE_ICONTEXT_OPERATOR_UNARYSTORE ;
                 regidx = icontext->iargs[idx+1];
                 qtrace_get_reg_offset_and_size(s, regidx, 
...
static inline void gen_movs(DisasContext *s, int ot)
{
    ...
    gen_op_ld_T0_A0(s, ot + s->mem_index);
    gen_string_movl_A0_EDI(s);
    gen_op_st_T0_A0(s, ot + s->mem_index);
...
{
    gen_string_movl_A0_EDI(s);
    gen_op_ld_T1_A0(s, ot + s->mem_index);
...
static void gen_op_used_regs(DisasContext *s1, int op, int ot, int d, uint16_t *rr, uint16_t *wr)
{
    ...
    case OP_ORL:
    case OP_XORL:
        if (d != OR_TMP0) *wr |= QTRACE_REG_SHIFT(d);
        break;
    default:
        break;
    }
...
static void gen_op(DisasContext *s1, int op, int ot, int d)
{
    if (d != OR_TMP0) {
        ...
        gen_op_ld_T0_A0(s1, ot + s1->mem_index);
    }
    switch(op) {
    case OP_ADCL:
        ...
        if (d != OR_TMP0)
            ...
            gen_op_st_T0_A0(s1, ot + s1->mem_index);
                                    ...
                                    int *reg_ptr, int *offset_ptr, uint16_t* rr, uint16_t* wr)
{
    ...
    if (s->aflag) {
        ...
        if (index >= 0) {
            if (scale == 0) {
                *rr |= QTRACE_REG_SHIFT(index);
            } else {
                *rr |= QTRACE_REG_SHIFT(index);
            }
            if (base >= 0) {
                *rr |= QTRACE_REG_SHIFT(base);
            }
        } else if (base >= 0) {
            *rr |= QTRACE_REG_SHIFT(base);
        }
        ...
        switch(rm) {
        case 0:
            *rr |= QTRACE_REG_SHIFT(R_EBX);
            *rr |= QTRACE_REG_SHIFT(R_ESI);
            break;
        case 1:
            *rr |= R_EBX;
            *rr |= R_EDI;
...
/* similar to gen_ldst_modrm, but only come up with a list of read/write registers
   without generating the emulation instructions */
static void gen_ldst_modrm_used_regs(CPUX86State *env, DisasContext *s, int modrm,
                                     ...
                                     uint16_t *wr)
{
    ...
    if (mod == 3) {
        ...
        gen_lea_modrm_used_regs(env, s, modrm, &opreg, &disp, rr, wr);
        if (is_store) {
            if (reg != OR_TMP0)
                *rr |= QTRACE_REG_SHIFT(reg);
            gen_op_st_T0_A0(s, ot + s->mem_index);
...
static void gen_push_T0(DisasContext *s)
{
    ...
    if (CODE64(s)) {
        ...
        if (s->dflag) {
            gen_op_addq_A0_im(-8);
            gen_op_st_T0_A0(s, OT_QUAD + s->mem_index);
        } else {
            gen_op_addq_A0_im(-2);
            gen_op_st_T0_A0(s, OT_WORD + s->mem_index);
        }
...
#endif
    {
        ...
        gen_op_st_T0_A0(s, s->dflag + 1 + s->mem_index);
        if (s->ss32 && !s->addseg)
...
static void gen_push_T1(DisasContext *s)
{
    ...
    if (CODE64(s)) {
        ...
        if (s->dflag) {
            gen_op_addq_A0_im(-8);
            gen_op_st_T1_A0(s, OT_QUAD + s->mem_index);
        } else {
            gen_op_addq_A0_im(-2);
            gen_op_st_T0_A0(s, OT_WORD + s->mem_index);
        }
...
#endif
    {
        ...
        gen_op_st_T1_A0(s, s->dflag + 1 + s->mem_index);
...
static void gen_pop_T0(DisasContext *s)
{
    ...
    if (CODE64(s)) {
        gen_op_movq_A0_reg(R_ESP);
        gen_op_ld_T0_A0(s, (s->dflag ? OT_QUAD : OT_WORD) + s->mem_index);
    } else
#endif
    {
        gen_op_movl_A0_reg(R_ESP);
        ...
        gen_op_ld_T0_A0(s, s->dflag + 1 + s->mem_index);
    }
}
...
static void gen_pusha(DisasContext *s)
{
    ...
    for(i = 0;i < 8; i++) {
        ...
        gen_op_st_T0_A0(s, OT_WORD + s->dflag + s->mem_index);
...
static void gen_popa(DisasContext *s)
{
    ...
    for(i = 0;i < 8; i++) {
        /* ESP is not reloaded */
        if (i != 3) {
            gen_op_ld_T0_A0(s, OT_WORD + s->dflag + s->mem_index);
    ...
    if (sse_fn_epp == SSE_SPECIAL) {
        ...
        case 0x32b: /* movntsd */
            ...
            if (b1 & 1) {
                ...
                gen_op_st_T0_A0(s, OT_LONG + s->mem_index);
            }
            break;
        ...
        case 0x210: /* movss xmm, ea */
            if (mod != 3) {
                gen_lea_modrm(env, s, modrm, &reg_addr, &offset_addr);
                gen_op_ld_T0_A0(s, OT_LONG + s->mem_index);
                               ...
                               target_ulong pc_start)
{
 ...
 reswitch:
    ...
    case 0x0f:
        ...
        /* arith & logic */
    case 0x00 ... 0x05:  /* add */
    case 0x08 ... 0x0d:  /* logic or */ 
    case 0x10 ... 0x15:  /* add with carry */
    case 0x18 ... 0x1d:  /* sub with borrow */
    case 0x20 ... 0x25:  /* logic and */
    case 0x28 ... 0x2d:  /* substract */
    case 0x30 ... 0x35:  /* logic exclusive or */
    case 0x38 ... 0x3d:  /* cmp */
        {
            QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_ARITHLOGIC);
            int op, f, val;
            op = (b >> 3) & 7;
            f = (b >> 1) & 3;
            ...
            case 0: /* OP Ev, Gv */
                ...
                rm = (modrm & 7) | REX_B(s);
                if (mod != 3) {
                    QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_FETCH);
                    /* cmp Ev, Gv does not do a store ! */
                    QTRACE_ADD_COND_INST_TYPE_FLAG(s, QTRACE_IS_X86_STORE, op != OP_CMPL);
                    gen_lea_modrm_used_regs(env, s, modrm, &reg_addr, &offset_addr, &rr, &wr);
                    QTRACE_ADD_INST_RR_FLAG_NOSHIFT(s, rr);
                    QTRACE_ADD_INST_RR_FLAG(s, reg);
                    QTRACE_ADD_INST_WR_FLAG_NOSHIFT(s, wr);
                    gen_lea_modrm(env, s, modrm, &reg_addr, &offset_addr);
                    opreg = OR_TMP0;
                ...
                xor_zero:
                    ...
                    gen_op_used_regs(s, op, ot, rm, &rr, &wr);
                    QTRACE_ADD_INST_RR_FLAG_NOSHIFT(s, rr);
                    QTRACE_ADD_INST_WR_FLAG_NOSHIFT(s, wr);
                    QTRACE_ADD_INST_RR_FLAG(s, reg);
                    QTRACE_CLIENT_MODULE(s);
            ...
            case 1: /* OP Gv, Ev */
                ...
                if (mod != 3) {
                    QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_FETCH);
                    gen_lea_modrm_used_regs(env, s, modrm, &reg_addr, &offset_addr, &rr, &wr);
                    gen_op_used_regs(s, op, ot, reg, &rr, &wr);
                    QTRACE_ADD_INST_RR_FLAG_NOSHIFT(s, rr);
                    QTRACE_ADD_INST_WR_FLAG_NOSHIFT(s, wr);
                    QTRACE_CLIENT_MODULE(s);
                    gen_lea_modrm(env, s, modrm, &reg_addr, &offset_addr);
                ...
                break;
            case 2: /* OP A, Iv */
                QTRACE_ADD_INST_RR_FLAG(s, OR_EAX);
                QTRACE_ADD_COND_INST_WR_FLAG(s, OR_EAX, (op!=OP_CMPL));
                QTRACE_CLIENT_MODULE(s);
    ...
    case 0x40 ... 0x47: /* inc Gv */
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_ARITHLOGIC);
        QTRACE_ADD_INST_RR_FLAG(s, OR_EAX + (b & 7));
        QTRACE_ADD_INST_WR_FLAG(s, OR_EAX + (b & 7));
        QTRACE_CLIENT_MODULE(s);
    ...
    case 0xf7:
        ...
        if (mod != 3) {
            QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_FETCH);
            QTRACE_ADD_COND_INST_TYPE_FLAG(s, QTRACE_IS_X86_STORE, (op >=1 && op<=3));
            gen_lea_modrm_used_regs(env, s, modrm, &reg_addr, &offset_addr, &rr, &wr);
            QTRACE_ADD_INST_RR_FLAG_NOSHIFT(s, rr);
            QTRACE_CLIENT_MODULE(s);
            if (op == 0)
            ...
            QTRACE_ADD_INST_RR_FLAG(s, rm);
            QTRACE_ADD_COND_INST_RR_FLAG(s, rm, (op == 2 || op == 3));
            QTRACE_CLIENT_MODULE(s);
            gen_op_mov_TN_reg(ot, 0, rm);
        }
    ...
    case 0xff: /* GRP5 */
        ...
        QTRACE_ADD_COND_INST_TYPE_FLAG(s, QTRACE_IS_X86_ARITHLOGIC, op <= 1);
        QTRACE_ADD_COND_INST_TYPE_FLAG(s, QTRACE_IS_X86_CALL&QTRACE_IS_X86_INDIRECT,  op == 2);
        QTRACE_ADD_COND_INST_TYPE_FLAG(s, QTRACE_IS_X86_JMP&QTRACE_IS_X86_INDIRECT,   op == 4);
        QTRACE_ADD_COND_INST_TYPE_FLAG(s, QTRACE_IS_X86_LCALL&QTRACE_IS_X86_INDIRECT, op == 3);
        QTRACE_ADD_COND_INST_TYPE_FLAG(s, QTRACE_IS_X86_LJMP&QTRACE_IS_X86_INDIRECT,  op == 5);
        if (mod != 3) {
            /* push does not fetch from memory */
            QTRACE_ADD_COND_INST_TYPE_FLAG(s, QTRACE_IS_X86_FETCH, (op != 6));
            QTRACE_ADD_COND_INST_TYPE_FLAG(s, QTRACE_IS_X86_STORE, (op == 0 || op == 1));
            QTRACE_ADD_INST_RR_ADDR_FLAG(s);
            QTRACE_CLIENT_MODULE(s);
            gen_lea_modrm(env, s, modrm, &reg_addr, &offset_addr);
            if (op >= 2 && op != 3 && op != 5)
                gen_op_ld_T0_A0(s, ot + s->mem_index);
            ...
            QTRACE_ADD_INST_RR_FLAG(s, rm);
            QTRACE_ADD_COND_INST_WR_FLAG(s, rm, (op == 0 || op == 1));
            QTRACE_CLIENT_MODULE(s);
            gen_op_mov_TN_reg(ot, 0, rm);
        }
        ...
        case 3: /* lcall Ev */
            ...
            gen_op_ldu_T0_A0(s, OT_WORD + s->mem_index);
        do_lcall:
            if (s->pe && !s->vm86) {
                gen_update_cc_op(s);
                gen_jmp_im(pc_start - s->cs_base);
    ...
    case 0x85:
        ...
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_ARITHLOGIC);
        QTRACE_ADD_COND_INST_TYPE_FLAG(s, QTRACE_IS_X86_FETCH, test_ldst_mem(modrm));
        gen_ldst_modrm_used_regs(env, s, modrm, ot, OR_TMP0, 0, &rr, &wr);
        QTRACE_ADD_INST_RR_FLAG_NOSHIFT(s, rr);
        QTRACE_ADD_INST_WR_FLAG_NOSHIFT(s, wr);
        QTRACE_CLIENT_MODULE(s);
    ...
    case 0x6b:
        ...
        QTRACE_ADD_INST_RR_FLAG_NOSHIFT(s, rr);
        QTRACE_ADD_INST_WR_FLAG_NOSHIFT(s, wr);
        QTRACE_ADD_COND_INST_RR_FLAG(s, reg, (b == 0x1af));
        QTRACE_ADD_INST_WR_FLAG(s, reg);
        QTRACE_CLIENT_MODULE(s);
    ...
    case 0x1c1: /* xadd Ev, Gv */
        ...
        if (mod == 3) {
            ...
            QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_FETCH);
            QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_STORE);
            QTRACE_CLIENT_MODULE(s);
            gen_lea_modrm(env, s, modrm, &reg_addr, &offset_addr);
            gen_op_mov_TN_reg(ot, 0, reg);
            gen_op_ld_T1_A0(s, ot + s->mem_index);
    ...
    case 0x1b1: /* cmpxchg Ev, Gv */
        {
            ...
            QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_CMPXCHG);
            QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_FETCH);
            QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_STORE);
            QTRACE_CLIENT_MODULE(s);
            ...
            if (mod == 3) {
                ...
                gen_op_ld_v(s, ot + s->mem_index, t0, a0);
                rm = 0; /* avoid warning */
            }
            ...
            if (mod == 3) {
                ...
                gen_op_st_v(s, ot + s->mem_index, t0, a0);
                gen_op_mov_reg_v(ot, R_EAX, t0);
                tcg_gen_br(label2);
                gen_set_label(label1);
    ...
    case 0x1c7: /* cmpxchg8b */
        ...
        /* push/pop */
    case 0x50 ... 0x57: /* push */
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_PUSH);
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_STORE);
        QTRACE_ADD_INST_RR_FLAG(s, (b & 7) | REX_B(s));
        QTRACE_CLIENT_MODULE(s);
        ...
        break;
    case 0x58 ... 0x5f: /* pop */
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_POP);
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_FETCH);
        QTRACE_ADD_INST_WR_FLAG(s, (b & 7) | REX_B(s));
        QTRACE_CLIENT_MODULE(s);
    ...
    case 0x60: /* pusha */
        ...
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_PUSH);
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_STORE);
        QTRACE_ADD_INST_RR_FLAG(s, R_EAX);
        QTRACE_ADD_INST_RR_FLAG(s, R_EBX);
        QTRACE_ADD_INST_RR_FLAG(s, R_ECX);
        QTRACE_ADD_INST_RR_FLAG(s, R_EDX);
        QTRACE_ADD_INST_RR_FLAG(s, R_ESI);
        QTRACE_ADD_INST_RR_FLAG(s, R_EDI);
        QTRACE_ADD_INST_RR_FLAG(s, R_EBP);
        QTRACE_ADD_INST_RR_FLAG(s, R_ESP);
        QTRACE_CLIENT_MODULE(s);
    ...
    case 0x61: /* popa */
        ...
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_POP);
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_FETCH);
        QTRACE_ADD_INST_WR_FLAG(s, R_EAX);
        QTRACE_ADD_INST_WR_FLAG(s, R_EBX);
        QTRACE_ADD_INST_WR_FLAG(s, R_ECX);
        QTRACE_ADD_INST_WR_FLAG(s, R_EDX);
        QTRACE_ADD_INST_WR_FLAG(s, R_ESI);
        QTRACE_ADD_INST_WR_FLAG(s, R_EDI);
        QTRACE_ADD_INST_WR_FLAG(s, R_EBP);
        QTRACE_ADD_INST_WR_FLAG(s, R_ESP);
        QTRACE_CLIENT_MODULE(s);
    ...
    case 0x8f: /* pop Ev */
        ...
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_POP);
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_FETCH);
        QTRACE_ADD_COND_INST_TYPE_FLAG(s, QTRACE_IS_X86_STORE, test_ldst_mem(modrm));
        QTRACE_CLIENT_MODULE(s);
    ...
    case 0x89: /* mov Gv, Ev */
        ...
        QTRACE_ADD_COND_INST_TYPE_FLAG(s, QTRACE_IS_X86_STORE, test_ldst_mem(modrm));
        gen_ldst_modrm_used_regs(env, s, modrm, ot, reg, 1, &rr, &wr);
        QTRACE_ADD_INST_RR_FLAG_NOSHIFT(s, rr);
        QTRACE_ADD_INST_WR_FLAG_NOSHIFT(s, wr);
        QTRACE_CLIENT_MODULE(s);
    ...
    case 0xc7: /* mov Ev, Iv */
        ...
        if (mod != 3) {
            ...
            QTRACE_ADD_INST_WR_FLAG(s, (modrm & 7) | REX_B(s));
        } 
        QTRACE_CLIENT_MODULE(s);
    ...
    case 0x1bf: /* movswS Gv, Eb */
        {
            ...
            if (mod == 3) {
                ...
                gen_lea_modrm(env, s, modrm, &reg_addr, &offset_addr);
                if (b & 8) {
                    gen_op_lds_T0_A0(s, ot + s->mem_index);
                } else {
                    gen_op_ldu_T0_A0(s, ot + s->mem_index);
                }
                gen_op_mov_reg_T0(d_ot, reg);
            }
        }
    ...
    case 0xa3:
        {
            ...
            QTRACE_ADD_COND_INST_TYPE_FLAG(s, QTRACE_IS_X86_STORE,  (b&2));
            QTRACE_ADD_COND_INST_TYPE_FLAG(s, QTRACE_IS_X86_FETCH, !(b&2));
            QTRACE_ADD_COND_INST_RR_FLAG(s, R_EAX, (b&2));
            QTRACE_ADD_COND_INST_WR_FLAG(s, R_EAX, !(b&2));
            QTRACE_CLIENT_MODULE(s);
    ...
    case 0xd7: /* xlat */
        ...
        gen_op_ldu_T0_A0(s, OT_BYTE + s->mem_index);
        gen_op_mov_reg_T0(OT_BYTE, R_EAX);
        break;
    case 0xb0 ... 0xb7: /* mov R, Ib */
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_MOV);
        QTRACE_ADD_INST_WR_FLAG(s,  (b & 7) | REX_B(s));
        QTRACE_CLIENT_MODULE(s);
    ...
    do_xchg_reg_eax:
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_XCHG);
        ot = dflag + OT_WORD;
        reg = (b & 7) | REX_B(s);
        rm = R_EAX;
        QTRACE_ADD_INST_RR_FLAG(s, reg);
    ...
    case 0xd8 ... 0xdf:
        ...
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_FPOINT);
        ...
        if (mod != 3) {
            ...
            case 0x30 ... 0x37: /* fixxx */
                {
                    ...
                    case 3:
                    default:
                        gen_op_lds_T0_A0(s, OT_WORD + s->mem_index);
                        tcg_gen_trunc_tl_i32(cpu_tmp2_i32, cpu_T[0]);
                        gen_helper_fildl_FT0(cpu_env, cpu_tmp2_i32);
                        break;
                    }
            ...
            case 0x0d: /* fldcw mem */
                ...
                gen_op_ld_T0_A0(s, OT_WORD + s->mem_index);
                tcg_gen_trunc_tl_i32(cpu_tmp2_i32, cpu_T[0]);
    ...
    case 0x6d:
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_FETCH);
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_IO);
        QTRACE_CLIENT_MODULE(s);
    ...
    case 0xef:
        ...
        /* control */
    case 0xc2: /* ret im */
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_RETURN);
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_FETCH);
        QTRACE_CLIENT_MODULE(s);
    ...
    case 0xca: /* lret im */
        ...
        s->pc += 2;
    do_lret:
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_LRETURN);
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_FETCH);
        QTRACE_CLIENT_MODULE(s);
        if (s->pe && !s->vm86) {
            gen_update_cc_op(s);
            ...
            gen_stack_A0(s);
            /* pop offset */
            gen_op_ld_T0_A0(s, 1 + s->dflag + s->mem_index);
            if (s->dflag == 0)
                gen_op_andl_T0_ffff();
    ...
    case 0xcb: /* lret */
        ...
        goto do_lret;
    case 0xcf: /* iret */
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_IRETURN);
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_FETCH);
        QTRACE_CLIENT_MODULE(s);
    ...
    case 0xe8: /* call im */
        {
            QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_CALL);
            QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_STORE);
            QTRACE_CLIENT_MODULE(s);
    ...
    case 0x9a: /* lcall im */
        ...
        goto do_lcall;
    case 0xe9: /* jmp im */
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_JMP);
        QTRACE_CLIENT_MODULE(s);
    ...
    case 0xea: /* ljmp im */
        {
            QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_LJMP);
            QTRACE_CLIENT_MODULE(s);
    ...
    do_jcc:
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_JMP);
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_COND);
        QTRACE_CLIENT_MODULE(s);
        next_eip = s->pc - s->cs_base;
        tval += next_eip;
        if (s->dflag == 0)
    ...
    case 0xd5: /* aad */
        ...
        /* misc */
    case 0x90: /* nop */
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_NOP);
        QTRACE_CLIENT_MODULE(s);
    ...
    case 0x9b: /* fwait */
        ...
        break;
    case 0xcc: /* int3 */
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_INT);
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_INT3);
        QTRACE_CLIENT_MODULE(s);
...
#endif
    case 0xfa: /* cli */
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_CLI);
        QTRACE_CLIENT_MODULE(s);
        if (!s->vm86) {
            if (s->cpl <= s->iopl) {
        ...
        break;
    case 0xfb: /* sti */
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_STI);
        QTRACE_CLIENT_MODULE(s);
        if (!s->vm86) {
            if (s->cpl <= s->iopl) {
            gen_sti:
    ...
    case 0x1c8 ... 0x1cf: /* bswap reg */
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_BSWAP);
        QTRACE_CLIENT_MODULE(s);
        reg = (b & 7) | REX_B(s);
#ifdef TARGET_X86_64
        if (dflag == 2) {
    ...
    case 0x130: /* wrmsr */
    case 0x132: /* rdmsr */
        QTRACE_ADD_COND_INST_TYPE_FLAG(s, QTRACE_IS_X86_RDMSR,  (b & 2));
        QTRACE_ADD_COND_INST_TYPE_FLAG(s, QTRACE_IS_X86_RDMSR, !(b & 2));
        QTRACE_CLIENT_MODULE(s);
        if (s->cpl != 0) {
            gen_exception(s, EXCP0D_GPF, pc_start - s->cs_base);
        ...
        break;
    case 0x131: /* rdtsc */
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_RDTSC);
        QTRACE_CLIENT_MODULE(s);
        gen_update_cc_op(s);
        gen_jmp_im(pc_start - s->cs_base);
        if (use_icount)
    ...
    case 0x133: /* rdpmc */
        ...
        break;
    case 0x134: /* sysenter */
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_SYSENTER);
        QTRACE_CLIENT_MODULE(s);
        ...
        break;
    case 0x135: /* sysexit */
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_SYSEXIT);
        QTRACE_CLIENT_MODULE(s);
    ...
    case 0x105: /* syscall */
        ...
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_SYSCALL);
        QTRACE_CLIENT_MODULE(s);
        gen_update_cc_op(s);
        gen_jmp_im(pc_start - s->cs_base);
        ...
        break;
    case 0x107: /* sysret */
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_SYSRET);
        QTRACE_CLIENT_MODULE(s);
        if (!s->pe) {
            gen_exception(s, EXCP0D_GPF, pc_start - s->cs_base);
        } else {
...
#endif
    case 0x1a2: /* cpuid */
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_CPUID);
        QTRACE_CLIENT_MODULE(s);
        gen_update_cc_op(s);
        gen_jmp_im(pc_start - s->cs_base);
        ...
        break;
    case 0xf4: /* hlt */
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_HLT);
        QTRACE_CLIENT_MODULE(s);
        if (s->cpl != 0) {
            gen_exception(s, EXCP0D_GPF, pc_start - s->cs_base);
        } else {
    ...
    case 0x101:
        ...
        case 0: /* sgdt */
            ...
            gen_op_st_T0_A0(s, CODE64(s) + OT_LONG + s->mem_index);
            break;
        case 1:
            if (mod == 3) {
                switch (rm) {
        ...
        case 3: /* lidt */
            if (mod == 3) {
                ...
                case 6: /* SKINIT */
                    ...
                    break;
                case 7: /* INVLPGA */
                    QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_INVLPGA);
                    ///QTRACE_CLIENT_MODULE(s);
                    if (!(s->flags & HF_SVME_MASK) || !s->pe)
                        goto illegal_op;
                    if (s->cpl != 0) {
                ...
                gen_lea_modrm(env, s, modrm, &reg_addr, &offset_addr);
                gen_op_ld_T1_A0(s, OT_WORD + s->mem_index);
                gen_add_A0_im(s, 2);
                gen_op_ld_T0_A0(s, CODE64(s) + OT_LONG + s->mem_index);
                if (!s->dflag)
                    gen_op_andl_T0_im(0xffffff);
                if (op == 2) {
    ...
    case 0x108: /* invd */
    case 0x109: /* wbinvd */
        QTRACE_ADD_COND_INST_TYPE_FLAG(s, QTRACE_IS_X86_INVD, (b & 2));
        QTRACE_ADD_COND_INST_TYPE_FLAG(s, QTRACE_IS_X86_WBINVD, (b & 2));
        QTRACE_CLIENT_MODULE(s);
        if (s->cpl != 0) {
            gen_exception(s, EXCP0D_GPF, pc_start - s->cs_base);
        } else {
    ...
    case 0x63: /* arpl or movslS (x86_64) */
        ...
        if (CODE64(s)) {
            ...
            if (mod == 3) {
                ...
                gen_lea_modrm(env, s, modrm, &reg_addr, &offset_addr);
                if (d_ot == OT_QUAD) {
                    gen_op_lds_T0_A0(s, OT_LONG + s->mem_index);
                } else {
                    gen_op_ld_T0_A0(s, OT_LONG + s->mem_index);
                }
                gen_op_mov_reg_T0(d_ot, reg);
...
#endif
        {
            ...
            if (mod != 3) {
                gen_lea_modrm(env, s, modrm, &reg_addr, &offset_addr);
                gen_op_ld_v(s, ot + s->mem_index, t0, cpu_A0);
                a0 = tcg_temp_local_new();
                tcg_gen_mov_tl(a0, cpu_A0);
            } else {
                gen_op_mov_v_reg(ot, t0, rm);
    ...
    case 0x103: /* lsl */
        ...
        break;
    case 0x118:
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_PREFETCH);
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_FETCH);
        QTRACE_CLIENT_MODULE(s);
        modrm = cpu_ldub_code(env, s->pc++);
        mod = (modrm >> 6) & 3;
    ...
    case 0x122: /* mov crN, reg */
        QTRACE_ADD_COND_INST_TYPE_FLAG(s, QTRACE_IS_X86_WRITE_CR,  (b & 2));
        QTRACE_ADD_COND_INST_TYPE_FLAG(s, QTRACE_IS_X86_FETCH_CR, !(b & 2));
        QTRACE_CLIENT_MODULE(s);
        if (s->cpl != 0) {
            gen_exception(s, EXCP0D_GPF, pc_start - s->cs_base);
        } else {
    ...
    case 0x123: /* mov drN, reg */
        QTRACE_ADD_COND_INST_TYPE_FLAG(s, QTRACE_IS_X86_WRITE_DR,  (b & 2));
        QTRACE_ADD_COND_INST_TYPE_FLAG(s, QTRACE_IS_X86_FETCH_DR, !(b & 2));
        QTRACE_CLIENT_MODULE(s);
        if (s->cpl != 0) {
            gen_exception(s, EXCP0D_GPF, pc_start - s->cs_base);
        } else {
    ...
    case 0x1ae:
        ...
        QTRACE_ADD_COND_INST_TYPE_FLAG(s, QTRACE_IS_X86_FXSAVE,  (op == 0));
        QTRACE_ADD_COND_INST_TYPE_FLAG(s, QTRACE_IS_X86_FXRSTOR, (op == 1));
        QTRACE_ADD_COND_INST_TYPE_FLAG(s, QTRACE_IS_X86_MEMFENCE,(op == 5 || op == 6));
        QTRACE_ADD_COND_INST_TYPE_FLAG(s, QTRACE_IS_X86_CLFLUSH, (op == 7 && (modrm & 0xc7) != 0xc0));
        QTRACE_CLIENT_MODULE(s);
        switch(op) {
        case 0: /* fxsave */
    ...
    case 0x1c4 ... 0x1c6:
    case 0x1d0 ... 0x1fe:
        QTRACE_ADD_INST_TYPE_FLAG(s, QTRACE_IS_X86_SSE);
        QTRACE_CLIENT_MODULE(s);
    ...
    /* QTRACE - update pc, linear pc and physical pc */
...
void optimize_flags_init(void)
{
    ...
    cpu_senv = tcg_global_reg_new_ptr(TCG_AREGS, "senv");
                                                  ...
                                                  bool search_pc)
{
    ...
    for(;;) {
        ...
        is_kern = (dc->cpl == 0);
        is_user = (dc->cpl >  0);
        if (dc->cpl != 0 && dc->cpl != 3) QTRACE_WARN("unknown priviledge level\n");
        if (is_kern) QTRACE_ADD_INST_TYPE_FLAG(dc, QTRACE_IS_X86_KERN);
        if (is_user) QTRACE_ADD_INST_TYPE_FLAG(dc, QTRACE_IS_X86_USER);
    ...
    *tcg_ctx.gen_opc_ptr = INDEX_op_end;
    ...
    if (qemu_loglevel_mask(QTRACE_LOG_FLAGS)) {
        qemu_log("----------------\n");
        qemu_log("QTRACE FLAGS:\n");
        qemu_log("{\n");
        qemu_log("  qtrace_insnflags ==> %s\n", lookup_x86_insn_text(dc, dc->qtrace_flags.qtrace_insnflags_str, dc->qtrace_flags.qtrace_insnflags));
        qemu_log("  qtrace_insnrr ==> %s\n", lookup_x86_gpr_text(dc, dc->qtrace_flags.qtrace_fetch_gpr_str, dc->qtrace_flags.qtrace_insnrr));
        qemu_log("  qtrace_insnwr ==> %s\n", lookup_x86_gpr_text(dc, dc->qtrace_flags.qtrace_write_gpr_str, dc->qtrace_flags.qtrace_insnwr));
        qemu_log("}\n");
    } 
...
void restore_state_to_opc(CPUX86State *env, TranslationBlock *tb, int pc_pos)
{
    ...
    if (cc_op != CC_OP_DYNAMIC)
        env->cc_op = cc_op;
                 ...
                 if (regidx == QTRACE_X86_RIP && QTRACE_ISPREINST_INSTRUMENT(icontext)) gen_op_sync_preinst_pc(s); 
                 if (regidx == R_LRIP && QTRACE_ISPREINST_INSTRUMENT(icontext)) gen_op_sync_preinst_lpc(s); 
                 if (regidx == R_PRIP && QTRACE_ISPREINST_INSTRUMENT(icontext)) gen_op_sync_preinst_ppc(s); 
